...
def crawling(news_type):
    ...
    if news_type == 'today_main_news':
        ...
        try :
            ...
            pass
        for k in range(1, 8):
            driver.find_element_by_xpath('/html/body/div[1]/table/tbody/tr/td[2]/div/div/div[7]/a['+str(k)+']').click()
            driver.implicitly_wait(2)
            for i in range(1,5):
                for j in range(1,6):
            try:
                driver.find_element_by_xpath('/html/body/div[1]/table/tbody/tr/td[2]/div/div/div[5]/ul['+str(i)+']/li['+str(j)+']/dl/dt[2]/a').click() 
                driver.implicitly_wait(2)
                data_list.append(get_news_info_df(news_type))
                driver.back()
            except:
